...
void msort(Student * base, int nel, int (*compar)(const void *, const void *)) {
	...
	Student* midpoint = base + (nel/2);
	
	//2a. Sort the first half of the array (remember to adjust the # elements)

	// 2 elements -> first do 1 element, then do 1, midpoint is index 1
	// 3 elements -> first do 1 element, then do 2, midpoint is index 1
	//
	...
	msort(midpoint,(nel + 1) / 2,compar);
	
	//3a. Merge the two arrays (use merge)

	Student* merged = merge(base,nel / 2,midpoint,(nel + 1) / 2,compar);	
...
Student * merge(Student * base1, int nel1, Student * base2, int nel2, int (*compar)(const void *, const void *)) {
	...
	for (int i = 0; i < (nel1 + nel2); i++)
	{
		...
		else if (compar(&base1[ind1],&base2[ind2]) < 0)
		{
			merged[i] = base1[ind1];
			ind1++;
		}
